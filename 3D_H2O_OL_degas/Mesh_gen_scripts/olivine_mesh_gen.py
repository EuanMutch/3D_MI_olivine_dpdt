...
f_mesh = sys.argv[1]
f_coords = sys.argv[2]
mesh_lres = float(sys.argv[3])
mesh_hres = float(sys.argv[4])
...
z = float(sys.argv[5]) # half length in microns
mi_r = float(sys.argv[6]) # # radius of mi inclusion - microns
tol = mi_r/2.0  # edge tolerance
...
while mi_test == False:
    ...
    mi_x = float(sys.argv[7]) #random.uniform(min_x, max_x) melt inclusion location currently set to 0
    mi_y = float(sys.argv[8]) #random.uniform(min_y, max_y) melt inclusion location currently set to 0
    mi_z = float(sys.argv[9]) #random.uniform(min_z, max_z) melt inclusion location currently set to 0

    face_test = np.empty([0])

    for x in ol_faces:
        pl_par = sphg.pl_eq(x[0], x[1], x[2])
